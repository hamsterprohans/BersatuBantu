...
Future<void> testDashboardQuery() async {
  try {
    ...
    if (pendingRecords.isNotEmpty) {
      ...
      final firstRecord = pendingRecords[0];
      ...
      firstRecord.forEach((key, value) {
        ...
        final shortened = valueStr.length > 50
            ? '${valueStr.substring(0, 50)}...'
            : valueStr;
        print(
          '[TEST 3]   "$key": "$shortened" (type: ${value.runtimeType}, isNull: ${value == null}, isEmpty: ${value is String ? value.isEmpty : "N/A"})',
        );
      });

      print('\n[TEST 3] Expected fields check:');
      print(
        '[TEST 3]   request_id exists: ${firstRecord.containsKey('request_id')}, value: ${firstRecord['request_id']}',
      );
      print(
        '[TEST 3]   nama_organisasi exists: ${firstRecord.containsKey('nama_organisasi')}, value: ${firstRecord['nama_organisasi']}',
      );
      print(
        '[TEST 3]   nama_pemilik exists: ${firstRecord.containsKey('nama_pemilik')}, value: ${firstRecord['nama_pemilik']}',
      );
      print(
        '[TEST 3]   email_organisasi exists: ${firstRecord.containsKey('email_organisasi')}, value: ${firstRecord['email_organisasi']}',
      );
      print(
        '[TEST 3]   email_pemilik exists: ${firstRecord.containsKey('email_pemilik')}, value: ${firstRecord['email_pemilik']}',
      );
      print(
        '[TEST 3]   no_telpon_pemilik exists: ${firstRecord.containsKey('no_telpon_pemilik')}, value: ${firstRecord['no_telpon_pemilik']}',
      );
      print(
        '[TEST 3]   no_telpon_organisasi exists: ${firstRecord.containsKey('no_telpon_organisasi')}, value: ${firstRecord['no_telpon_organisasi']}',
      );
      print(
        '[TEST 3]   status exists: ${firstRecord.containsKey('status')}, value: ${firstRecord['status']}',
      );
      ...
      try {
        final explicitFields = await supabase
            .from('organization_request')
            .select(
              'nama_organisasi, nama_pemilik, email_organisasi, no_telpon_organisasi',
            )
            .eq('status', 'pending')
            .limit(1);
      ...
      if (allRecords.isNotEmpty) {
        ...
        print('[TEST 3] First record keys: ${(allRecords[0]).keys.toList()}');
      }
    }
